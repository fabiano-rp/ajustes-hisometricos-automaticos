...
```
ROTINA PARA AUTOMATIZAÇÃO DE AJUSTES HIPSOMÉTRICOS POR PARCELA

BASE DE DADOS OBTIDA DE PLANTIOS DE *Eucalyptus urophylla*, CLONE AEC 0144

Autor: Fabiano Rodrigues Pereira
...
```{r}
MSAIDA2 <- matrix(nrow = NROW(unique(MDADOS$parcela))*NROW(MODELOS), ncol = 4, 0)
```
...
names(MSAIDA2) <- c("PARC", "MODELO", "ERRO", "R2aj")
```
...
for (i in unique(MDADOS$parcela)) { # Filtrando por parcela
  ...
  if(nrow(MTEMP2) != 0){ # Verificando se ha arvores nao medidas
    ...
    for (k in 1:nrow(MODELOS)) { # Ajustando modelos por parcela
      ...
      MSAIDA2$R2aj[nlinha] <- ESTAT_ATUAL$adj.r.squared # Adicionando valor de R2 aj a tabela de erro dos modelos
...
MSAIDA2 %>% ggplot(aes(factor(PARC), ERRO))+
  ...
  labs(x = 'Parcela', y = 'Erro padrão residual', color = "Modelo")+
  theme_light(15)
```
